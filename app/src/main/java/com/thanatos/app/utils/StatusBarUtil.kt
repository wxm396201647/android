package com.thanatos.app.utils
...
class StatusBarUtil {
    ...
    companion object {
        ...
        private val isMIUI6Later: Boolean
            get() {
                return try {
                    ...
                    var `val` = mtd.invoke(null, "ro.miui.ui.version.cityName") as String
        ...
        fun darkMode(activity: Activity, color: Int = DEFAULT_COLOR, isBlack: Boolean = true) {
            darkMode(activity.window, color, DEFAULT_ALPHA, isBlack = isBlack)
        }
        ...
        /** 设置状态栏darkMode,字体颜色及icon变黑(目前支持MIUI6以上,Flyme4以上,Android M以上)  */
        @TargetApi(Build.VERSION_CODES.M)
        fun darkMode(window: Window, color: Int, @FloatRange(from = 0.0, to = 1.0) alpha: Float, isBlack: Boolean = true) {
            when {
                isFlyme4Later -> {
                ...
                isMIUI6Later -> {
                    darkModeForMIUI6(window, isBlack)
                    immersive(window, color, alpha)
                }
                Build.VERSION.SDK_INT >= Build.VERSION_CODES.M -> {
                    darkModeForM(window, isBlack)
                    immersive(window, color, alpha)
                }
            ...
            return try {
                ...
                if (Build.VERSION.SDK_INT >= 23) {
                    if (darkmode) {
                        ...
                        var flag: Int = window.decorView.systemUiVisibility or View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR.inv()
